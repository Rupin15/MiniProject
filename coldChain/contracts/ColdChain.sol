...
library CryptoSuite{
    ...
    function recoverSigner(bytes32 message,bytes memory sig) internal pure returns (address){
        ...
        bytes memory prefix="\x19Etherium Signed Message:\n32";
        bytes32 prefixHashed=keccak256(abi.encodePacked(prefix,message));
        return ecrecover(prefixHashed,v,r,s);
    }
...
contract ColdChain{
    ...
    //certificate is not verified more than it needs to
    uint public constant MAX_CERTIFICATIONS=2;

    uint[] public certificateIds;
    uint[] vaccineBatchIds;
...
//returns certificate ID
    function issueCertificate(address _issuer, address _prover,  string memory _status,uint vaccineBatchId,bytes memory signature) public returns (uint){
        ...
        require(issuer.mode==Mode.ISSUER,"Issuer is not there");

        Entity memory prover =entities[_prover];
        require(prover.mode==Mode.PROVER,"Prover is not there");
    ...
    function getModeOf(address _address) public view returns(Mode mode){
         Entity memory prover =entities[_address];
         return prover.mode;
    }
    ...
    function unmarshalStatus(string memory _status) private pure returns(Status status){
        ...
        else if(encodedStatus== encodedStatus1) return Status.DELIVERING_INTERNATIONAL;
        else if(encodedStatus==encodedStatus2) return Status.STORED;
        else if(encodedStatus== encodedStatus3) return Status.DELIVERING_LOCAL;
        else if(encodedStatus==encodedStatus4) return Status.DELIVERED;
